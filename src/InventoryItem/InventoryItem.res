...
@react.component
let make = (~item: ExecutorHook.InventoryItem.t, ~active = ?) => {
  ...
  <a id={"item-" ++ Belt.Int.toString(id)} onClick={e => {
  ...
  className={path -> List.get(1) == Some(Belt.Int.toString(id)) ? "col-span-4 lg:col-span-8" : "" ++ " block"}>
    <button className="relative m-[1.5] flex flex-col">
      ...
      <div className="flex flex-row justify-between w-full bg-gray-300 text-white shadow-sm">
        <span className="drop-shadow-sm"><i className="light-icon-shopping-cart text-xl"></i> <span className="text-[0.5rem]" title="Quantity in cart">{{ quantity -> str }}</span></span>
        <span className="drop-shadow-sm"><i className="light-icon-checks text-xl"></i> <span className="text-[0.5rem]" title="Quantity available">{{ available -> str }}</span></span>
      </div>
      <div className="flex flex-col text-align-center w-full bg-white text-gray-300 rounded-sm m-[1.5] justify-self-end">
        <h2 className="w-full text-xs drop-shadow-sm text-gray-500">{title -> str}</h2>
        <p className={(active == Some(true) ? "" : "hidden ") ++ "text-xs m-2"}>{description -> str}</p>
      </div>
    </button>
  </a>
}
